...
<<<<<<< HEAD
	...
	pickupState: string;
	flashSale: boolean;
	expectedPickupDate: number; // Stored as a timestamp (milliseconds since epoch)
	vendorStatus: "Pending" | "Approved" | "Rejected";
	// PO fields managed by Supply Ops

	logistics: "Ex" | "For" | ""; // Logistics type, Ex: Ex-works, For: Forwarding
	vendorPOCName: string; // Name of the vendor's point of contact
	vendorPOCPhone: string; // Phone number of the vendor's point of contact
	returnDays: number; // Number of days for returns
	SORFlag: "SOR" | "Non-SOR" | ""; // Stock on Road (SOR) flag

	poTotalValue: number; // Total value of the Purchase Order (PO)
	poNumber: string | ""; // PO number can be null if not issued yet
	poStatus: string | ""; // PO status can be null if not issued yet
	dateOfIssue: number | null; // Date of issue of the PO, null if not issued
}

export interface VendorWithSkus {
	vendorId: string;
	items: Array<{
		requestId: string;
		sku: SKU;
